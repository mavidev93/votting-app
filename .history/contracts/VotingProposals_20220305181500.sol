...
function getResults() public view returns(Proposal[] memory) {
    Proposal[] highVotedProposal;
    uint maxVoteNumber =0;
    for (uint i =0;i <proposals.length; i++){
        if (maxVoteNumber < proposals[i].vote){
            //reset array
            delete highVotedProposal;
            maxVoteNumber = proposals[i].vote;
            highVotedProposal.push(proposals[i]);

        }

        else if( maxVoteNumber == proposals[i].vote){
            //if the max and current vote are equal keep array and push others
            highVotedProposal.push(proposals[i].vote);
        }
