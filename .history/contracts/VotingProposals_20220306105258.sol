...
function getResults() public view returns(uint[] memory) {
    ...
    uint[] memory highVotedProposalIndex;

    uint maxVoteNumber =0;
    for (uint i =0;i <proposals.length; i++){
        if (maxVoteNumber < proposals[i].vote){
            //reset array
            delete highVotedProposalIndex;
            maxVoteNumber = proposals[i].vote;
            highVotedProposalIndex[0]= i;

        }

        else if( maxVoteNumber == proposals[i].vote){
            //if the max and current vote are equal keep array and push current element
            highVotedProposalIndex[highVotedProposalIndex.length +1 ] =i;
        }

    }

    if (highVotedProposalIndex.length ==0){
        highVotedProposalIndex[0]=[29292]
    }

    return highVotedProposalIndex;
