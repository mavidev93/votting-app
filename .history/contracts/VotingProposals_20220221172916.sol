...
modifier onlyOwner(){
 ...
 //register voter 
function registerUser(address _) public onlyOwner {
    if(! contains(_voter)){
     registerdVoters[_voter] = true;
     userByAddress[_voter] =  User({userAddress:_voter,isVoted:false,isProposed:false,isRegistered:false}) ;
    }
 }
