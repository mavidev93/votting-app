...
function sendProposal(address _user,string memory _proposal) public onlyRegisteredUser(_user) {

 //cheack app session
  require(keccak256(abi.encodePacked(session)) == keccak256(abi.encodePacked("sendProposals"))
   , "not this session in app");

 
  if(userByAddress[_user].isProposed){
...
function  vote(address _user, address _voteFor) public onlyRegisteredUser(_user) {
      ...
      require(keccak256(abi.encodePacked(session)) == keccak256(abi.encodePacked("getVotes"))
   , "not this session in app");
