...
function registerUser(address _user) public onlyOwner {
    if(! contains(_user)){
     ...
     userByAddress[_user] =  User({userAddress:address( _user),isVoted:false,isProposed:false,isRegistered:false}) ;
    }
 }

function sendProposal(address _user,string storage _proposal) public {
