      ...
      <table className="table-auto border-collapse min-w-[50%] border border-slate-400">
        ...
        <tbody>
          {proposals?.map((p, i) => (
            <tr
              className={` p-2  ${
                session === "showResults" && p.isHighVote ? "bg-green-500" : ""
              }`}
              key={i}
            >
              ...
              <td className="border border-slate-300 text-center">
                <button
                  ...
                  className={
                    session === "showResults"
                      ? "bg-blue-500 text-white font-bold py-1 px-3 my-1 rounded opacity-50 cursor-not-allowed"
                      : "bg-teal-400 hover:bg-teal-600 text-white  py-1 px-3 my-1 rounded m-1 "
                  }
