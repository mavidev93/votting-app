...
const {session} = useSession();
  ...
  useEffect(() => {
    ...
    async function getProposals() {
      ...
      let maxVote=  proposalsArr.reduce(function(prevProposal, currentProposal) {
        return (prevProposal.vote > currentProposal.vote) ? prevProposal : currentProposal
    })
  ...
  return proposals.length > 0 ? (
    <div className="flex justify-center p-1">
      <table className="table-auto border-collapse min-w-[50%] border border-slate-400">
        ...
        <tbody>
          {proposals?.map((p, i) => (
            <tr className={` p-2  ${session==="showResults" && p.isHighVote  "bg-green-500"}`}key={i}  >
