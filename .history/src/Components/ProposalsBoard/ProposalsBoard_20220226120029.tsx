...
function ProposalsBoard() {
  ...
  useEffect(() => {
    ...
    const contract = new ethers.Contract(votingProposalsAddress,VotingProposals.abi,pro)
        ...
        <tbody>
          ...
          {proposals?.map((p) => (
            <tr className=" p-2" key={p.proposedBy}>
              ...
              <td className="border border-slate-300">
                <button className="bg-teal-400 hover:bg-teal-600 text-white  py-1 px-3 rounded m-1 ">
                  Vote
                </button>
              </td>
            </tr>
